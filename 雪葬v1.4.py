...
from PyQt5.QtWidgets import (
    ...
    QStyleFactory, QDialog, QGridLayout, QComboBox,
    QGraphicsView, QGraphicsScene, QGraphicsRectItem
)
from PyQt5.QtCore import Qt, QTimer, QSize
from PyQt5.QtGui import QIcon, QFont, QPalette, QColor, QMovie, QBrush, QPen, QLinearGradient, QPainter
...
class ResourceChart(QGraphicsView):
    def __init__(self, parent=None):
        super().__init__(parent)
    ...
    def update_chart(self):
        ...
        gradient.setColorAt(0, QColor(142, 45, 197))
        gradient.setColorAt(1, QColor(63, 81, 181))
        brush = QBrush(gradient)
        pen = QPen(QColor(100, 100, 100))
        
        for i, value in enumerate(self.data):
            bar_height = (value / max_value) * height
            rect = QGraphicsRectItem(i * bar_width, height - bar_height, bar_width - 2, bar_height)
            rect.setBrush(brush)
            rect.setPen(pen)
...
class SnowBurial(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("雪葬 - 进程冻结工具 v1.3")
        ...
        self.thread_handles = {}  # 存储线程句柄
        self.memory_protected = {}  # 存储内存保护状态
    ...
    def set_dark_theme(self):
        ...
        dark_palette.setColor(QPalette.Window, QColor(40, 40, 45))
    ...
    def init_ui(self):
        ...
        self.cpu_chart.setMinimumHeight(100)
        self.cpu_chart.setMaximumHeight(120)
        self.cpu_chart.setStyleSheet("background-color: #1E1E1E;")
        chart_layout.addWidget(QLabel("CPU历史:"))
        chart_layout.addWidget(self.cpu_chart)
        
        self.mem_chart = ResourceChart()
        self.mem_chart.setMinimumHeight(100)
        self.mem_chart.setMaximumHeight(120)
        self.mem_chart.setStyleSheet("background-color: #1E1E1E;")
        chart_layout.addWidget(QLabel("内存历史:"))
        chart_layout.addWidget(self.mem_chart)
        ...
        try:
            ...
            try:
                ...
                if ctypes.windll.version.VerQueryValueW(info, "\\StringFileInfo\\040904b0\\FileDescription", 
                    ...
                    # 尝试中文代码页
                    if ctypes.windll.version.VerQueryValueW(info, "\\StringFileInfo\\080404b0\\FileDescription", 
                                                          ctypes.byref(value), ctypes.byref(value_len)) == 0:
                        return "无描述信息"
                
                # 读取描述字符串
                description = ctypes.c_wchar_p(value.value).value
                return description if description else "无描述信息"
            finally:
            ...
            while True:
                try:
                    ...
                    try:
                        ...
                        if hProcess:
                            try:
                                ...
                                if psapi.GetProcessMemoryInfo(hProcess, ctypes.byref(pmc), ctypes.sizeof(pmc)):
                                    memory_usage = pmc.PrivateUsage  # 使用私有工作集大小
                            finally:
                                win32api.CloseHandle(hProcess)
                    except Exception as e:
                        self.status_label.setText(f"获取内存信息错误 (PID: {pid}): {str(e)}")
                        memory_usage = 0
                    ...
                    if pid in self.last_cpu_times:
                        ...
                        if time_diff > 0:
                            try:
                                current_cpu = self.get_cpu_time(pid)
                                cpu_percent = (current_cpu - last_cpu) / time_diff * 100
                            except Exception as e:
                                self.status_label.setText(f"计算CPU使用率错误 (PID: {pid}): {str(e)}")
                                cpu_percent = 0.0
                    ...
                    try:
                        ...
                        self.status_label.setText(f"获取CPU时间错误 (PID: {pid}): {str(e)}")
                    ...
                    self.status_label.setText(f"处理进程时出错: {str(e)}")
    ...
    def get_cpu_time(self, pid):
        ...
        try:
            ...
            try:
                ...
                if kernel32.GetProcessTimes(hProcess, ctypes.byref(creation_time), ctypes.byref(exit_time),
    ...
    def suspend_process(self, pid):
        ...
        try:
            ...
            raise Exception(f"挂起进程失败: {str(e)}")
    ...
    def resume_process(self, pid):
        ...
        try:
            ...
            raise Exception(f"恢复进程失败: {str(e)}")
    ...
    def kill_process(self, pid):
        ...
        try:
            hProcess = win32api.OpenProcess(PROCESS_TERMINATE, False, pid)
            if not hProcess:
                raise Exception("无法打开进程，可能需要管理员权限")
